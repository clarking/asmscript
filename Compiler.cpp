	...
	[[nodiscard]]  Error CompileStatement(const Parser::Statement &statement, MachineCode &code, std::unordered_map<size_t, std::string> &callTable) {
		...
		switch (statement.tag) {
			case StatementTag::Assignment: {
				...
				switch (stmt.source->tag) {
					case OperandTag::Register:
						Gen::EmitMov(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
						break;
					case OperandTag::Immediate:
						Gen::EmitMov(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
						break;
					default:
			...
			case StatementTag::Shorthand: {
				...
				switch (stmt.op) {
					case Operation::Add:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitAdd(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitAdd(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
					...
					case Operation::Sub:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitSub(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitSub(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
					...
					case Operation::Mul:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitImul(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitImul(stmt.dest, stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
					...
					case Operation::And:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitAnd(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitAnd(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
					...
					case Operation::Or:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitOr(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitOr(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
					...
					case Operation::Xor:
						switch (stmt.source->tag) {
							case OperandTag::Register:
								Gen::EmitXor(stmt.dest, dynamic_cast<const RegisterOperand &>(*stmt.source).reg, code);
								break;
							case OperandTag::Immediate:
								Gen::EmitXor(stmt.dest, dynamic_cast<const ImmediateOperand &>(*stmt.source).value, code);
								break;
							default:
								return Error{"Unsopported source argument type.", statement.pos};
						}
			...
			case StatementTag::Push:
				Gen::EmitPush(dynamic_cast<const Parser::RegisterStatement &>(statement).reg, code);
				break;
			case StatementTag::Pop:
				Gen::EmitPop(dynamic_cast<const Parser::RegisterStatement &>(statement).reg, code);
				break;
			default:
				return Error{"Statement not implemented in the compiler.", statement.pos};
		}
